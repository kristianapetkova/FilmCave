...
//all movies, series and blu-ray products
$conn=mysqli_connect("localhost","root","","filmcave");
if (!$conn) {
	die("Connection failed: " . mysqli_connect_error());
}
$call = mysqli_prepare($conn, "CALL get_all_movie_products()");
mysqli_stmt_execute($call);
//products.ID, products.POSTER, titles.TITLE, types.TYPE, products.PRODUCT_PRICE, products.RATING, products.YEAR, genres.GENRE, series.SERIE, languages.LANGUAGE, subtitles.SUBTITLE, products.PROMOTION_PERCENT
mysqli_stmt_bind_result($call,$id_product,$genre_poster,$genre_title,$genre_type,$genre_product_price,$genre_rating,$year, $genre,$series,$language,$subtitles,$promo_percent);
	...
	$array_subtitles=array();	

while (mysqli_stmt_fetch($call)) {
	array_push($array_id_product, $id_product);
	array_push($array_title, $genre_title);
	array_push($array_type, $genre_type);
	array_push($array_product_price, $genre_product_price);
	array_push($array_poster, $genre_poster);
	array_push($array_rating, $genre_rating);
	array_push($array_promo_percent, $promo_percent);

	array_push($array_year, $year);
	array_push($array_genre, $genre);
	array_push($array_series, $series);
	array_push($array_language, $language);
	array_push($array_subtitles, $subtitles);
}
...
<script>
    ...
    for ($i = 0 ; $i<count($array_title); $i++) { ?>
    	array_title.push("<?php echo $array_title[$i]; ?>" ); 
    	<?php
    }
	...
	for(var i =0;i<result;i++){
		...
		$('.add_to_fav').click(function() {
		    ...
		    if(temp.match(/favourite_promo/)){
		    	...
		    	console.log(array_id_product[pos]);
		    	document.getElementById("id_favourite").value=array_id_product[pos];
		...
		$('.buttons').click(function() {
		    ...
		    if(temp.match(/cart_promo/)){
		    	let pos=(temp.match(/\d/))[Object.keys(temp.match(/\d/))[0]];
		    	console.log(array_id_product[pos]);
		    	document.getElementById("id_cart").value=array_id_product[pos];
